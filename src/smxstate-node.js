...
`
	...
	function renderGraph(node, forceRedraw, errorCallback, successCallback) {
			...
			switch(req.params.method) {
				case 'getgraph':
					try {
						...
						renderGraph(node,
							req.body.hasOwnProperty("forceRedraw") ? req.body.forceRedraw === "true" : false,
							() => { res.sendStatus(500); },
							(smcat_svg) => { res.status(200).send(smcat_svg); }
						);
